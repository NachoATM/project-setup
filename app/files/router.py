from fastapi import APIRouter, Header, Body, HTTPException, UploadFile, Form, File
...
from files.persistence.minio_persistence.files import FilesMinioPersistenceService

router = APIRouter()
files_database = {}

files_storage_system = FilesMinioPersistenceService
...
@router.get("/{id}")
async def files_get(id:str, auth: str = Header(alias="auth")) -> dict[str, str]:
...
async def files_get(auth: str = Header(alias="auth")) -> dict[str, str]:
    introspect_response = await introspect(auth=auth)
    print(introspect_response)
    return files_database[id].dict()


@router.post("")
async def files_post(auth: str = Header(alias="auth"), input_body : CarlemanyFile = Body()) -> dict[str, Any]:
    current_id = str(uuid.uuid4())
    while current_id in files_database:
        current_id = str(uuid.uuid4())
    input_body.path=None
    introspect_response = await introspect(auth=auth)
...
async def files_id_post(
    ...
    remote_path = await files_storage_system.put_file(
        prefix + filename,
        filename,
    )
    ...
    current_file.file.path = remote_path
    return {
        "status": "ok",
...
async def files_id_delete(id:str, auth: str = Header(alias="auth")) -> dict[str, str]:
    introspect_response = await introspect(auth=auth)
    filename= id + ".pdf"
    await files_storage_system.remove_file(filename)
